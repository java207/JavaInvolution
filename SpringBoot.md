...
以上：Spring本身只定义了两种Scope，Singleton和Prototype，一开始会先判断bean存不存在，如果存在就直接返回，如果不存在调用`createBean`创建bean
...
protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
	...
	if (earlySingletonExposure) {
		if (logger.isTraceEnabled()) {
            ...
            //当正在创建A时，A依赖B，此时通过将A作为ObjectFactory放入单例工厂中进行early expose，此处B需要引用A，但A正在创建，从单例工厂拿到ObjectFactory，从而允许循环依赖
